...
{   int imgwidth,imgheight,imgdepth;
   ...
   const char newimgname[]="images/girlface_west.bmp";
   struct mask rbsnmask;
   signed char *tmp;
   int i;

   rbsnmask.cols=rbsnmask.rows=3;
   rbsnmask.data=(unsigned char*)malloc(9);

   int rb_north[]={-1,0,1,-2,0,2,-1,0,1};
   int rb_south[]={1,0,-1,2,0,-2,1,0,-1};
   int rb_east[]={-1,-2,-1,0,0,0,1,2,1};
   int rb_west[]={1,2,1,0,0,0,-1,-2,-1};
   tmp=(signed char*)rbsnmask.data;
   for(i=0;i<9;i++)
   {
       *tmp=rb_west[i];
       tmp++;
    ...
    convolve(imgheight,imgwidth,&rbsnmask,buf,buf2);
